package com.atguigu.day23;

import java.io.File;
import java.io.IOException;
import java.text.SimpleDateFormat;
import java.util.Date;

import org.junit.Test;

/*
 * 	I/O:
 * 	Input:输入
 * 	Output:输出
 * 
 * 	针对当前的应用程序，把数据输出到“控制台”、“文件”、“网络”、“打印机”
 * 	针对当前的应用程序，把“键盘输入”、“文件”、“网络” .... 输入数据
 * 
 * 	java.io.File类：文件和目录路径名的抽象表示形式。
 * 		文件和目录（文件夹）都使用File类的对象表示
 * 		并且表示文件或目录都要用对应的路径名来表示
 * 
 * 	1、获取文件的很常见属性
 * 	（1）
 * 		getPath()：构造器怎么指定，就是什么值
 * 		getAbsolutePath()：绝对路径
 * 		getCanonicalPath()：规范路径，会正确解析../
 * 
 * 	（2）getName():文件名
 * 	（3）length()：可以获取文件的大小，不能直接获取目录（文件夹）的大小
 * 	（4）lastModified()：毫秒数
 * 	（5）父目录:String getParent()
 * 			  File getParaentFile()
 * 		下一级：String[] list()//返回文件名，String类型
 * 			   File[] file1.listFiles()//返回绝对路径，File类型
 * 
 * 	（6）判断信息
 * 		exists()：是否存在
 * 		isFile()：是否是文件
 * 		isDirectory()：是否是目录（文件夹）
 * 
 */
public class TestFile {
	
	@Test
	public void test10() {
		File file1 = new File("E:/1.txt");
		//如果不存在，就全部是默认值
		System.out.println("是否存在：" + file1.exists());
		System.out.println("是文件吗：" + file1.isFile());
		System.out.println("是目录吗：" + file1.isDirectory());
		System.out.println("大小：" + file1.length());
		System.out.println("最后修改时间：" + file1.lastModified());
		
		//除了可以从“路径名”中获取的信息，不会是默认值
		System.out.println("路径：" + file1.getPath());
		System.out.println("文件名：" + file1.getName());
		
	}
	
	@Test
	public void test9() {
		File file1 = new File("E:/code/测试");
		System.out.println("是否存在：" + file1.exists());
		System.out.println("是文件吗：" + file1.isFile());
		System.out.println("是目录吗：" + file1.isDirectory());
	}
	
	@Test
	public void test8() {
		File file1 = new File("E:/code/测试/1.docx");
		System.out.println("是否存在：" + file1.exists());
		System.out.println("是文件吗：" + file1.isFile());
		System.out.println("是目录吗：" + file1.isDirectory());
	}
	
	@Test
	public void test7() {
		File file1 = new File("E:/code/测试");
		System.out.println("下一级：");
		String[] list = file1.list();//返回文件名，String类型
		for (String sub : list) {
			System.out.println(sub);
		}
		
		System.out.println();
		
		File[] listFiles = file1.listFiles();//返回绝对路径，File类型
		for (File sub : listFiles) {
			System.out.println(sub);
		}
	}
	
	@Test
	public void test6() {
		File file1 = new File("E:/code/测试/1.docx");
		System.out.println("父目录：" + file1.getParent());
	}
	
	@Test
	public void test5() {
		File file1 = new File("E:/code/测试/1.docx");
		System.out.println("大小：" + file1.length());//65134字节
		System.out.println("最后修改时间：" + file1.lastModified());//1597474638937毫秒
		
		//常用类的方法
		Date date = new Date(file1.lastModified());
		System.out.println("最后修改时间：" + date);
		
		SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss SSS");
		System.out.println("最后修改时间：" + sf.format(date));
		
		File dir = new File("E:/code/测试");//文件夹
		System.out.println("大小：" + dir.length());//0
	}
	
	@Test
	public void test4() {
		File file1 = new File("E:/code/测试/1.docx");
		System.out.println("文件名：" + file1.getName());
		
		String fileName = file1.getName();
		int index = fileName.lastIndexOf(".");
		String ext = fileName.substring(index);
		System.out.println("扩展名：" + ext);
		
		
	}
	
	@Test
	public void test3() throws IOException {
		File file1 = new File("../../javase/1.txt");
		File file2 = new File("/javase/1.txt");
		File file3 = new File("javase/1.txt");
		File file4 = new File("E:/code/测试/1.docx");
		
		System.out.println("path1 = " + file1.getPath());//和创建这个对象时，如何指定路径
		System.out.println("path2 = " + file2.getPath());
		System.out.println("path3 = " + file3.getPath());
		System.out.println("path4 = " + file4.getPath());
		System.out.println();
		
		System.out.println("AbsolutePath1 = " + file1.getAbsolutePath());//绝对路径
		System.out.println("AbsolutePath2 = " + file2.getAbsolutePath());
		System.out.println("AbsolutePath3 = " + file3.getAbsolutePath());
		System.out.println("AbsolutePath4 = " + file4.getAbsolutePath());
		System.out.println();
		
		System.out.println("CanonicalPath1 = " + file1.getCanonicalPath());//规范路径
		System.out.println("CanonicalPath2 = " + file2.getCanonicalPath());
		System.out.println("CanonicalPath3 = " + file3.getCanonicalPath());
		System.out.println("CanonicalPath4 = " + file4.getCanonicalPath());
		
	}
	
	@Test
	public void test2() {
		//E:\code\测试\1.docx
//		File file = new File("E:\\code\\测试\\1.docx");
		
		//建议使用/，因为这种表示方式，在不同的操作系统，网络中表示路径都支持/，而\只要windows
		File file = new File("E:/code/测试/1.docx");
		
//		File file = new File("E:" + File.separator + "code" + File.separator + "测试" + File.separator + "1.docx");
		
		System.out.println("路径：" + file.getPath());
	}
	
	@Test
	public void test1() {
		//E:\code\测试\1.docx
		File file = new File("E:\\code\\测试\\1.docx");
		
		//E:\code\测试
		File dir = new File("E:\\code\\测试");
		
	}

}
